...
describe("DiamondTest", async function () {
    ...
    it("should remove some test2 functions", async () => {
        ...
        // const functionsToLose = ["test2Func1()", "test2Func5()", "test2Func6()", "test2Func19()", "test2Func20()"]
        const selectors = getSelectors(test2Facet).get(["test2Func5()"])

        console.log("selectors chosen:", selectors)

        let selector = await diamondLoupeFacet.facetFunctionSelectors(addresses[8])
        assert.equal(selector.length, 20)

        let num25 = await test2Facet.test2Func5()
        assert.equal(num25, 25n)
        ...
        // selects = await diamondLoupeFacet.facetFunctionSelectors(addresses[8])
        // console.log("selectors:", selects)

        result = await diamondLoupeFacet.facetFunctionSelectors(addresses[8])
        assert.sameMembers([...result], getSelectors(test2Facet).remove(selectors))

        selector = await diamondLoupeFacet.facetFunctionSelectors(addresses[8])
        assert.equal(selector.length, 19)

        num25 = await test2Facet.test2Func5()
        assert.equal(num25, 25n)
    })
