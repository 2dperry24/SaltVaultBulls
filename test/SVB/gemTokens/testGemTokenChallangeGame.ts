...
describe("Gem Token Challenge Test", async function () {
    ...
    describe("Test Different Games Setups", function () {
        ...
        describe("Start The Games", function () {
            let score: any
            let expectedScore: any

            it("conduct game 1", async function () {
                ...
                score = await gemTokenChallengeFacet.connect(signers[10]).checkTokenScoreForWallet()
                expectedScore = calculateTokenValue(await svbGemTokens.walletOfOwner(signers[10]), TokenData, colorForMonth)
            ...
            it("conduct game 2", async function () {
                console.log()
                console.log()
                console.log()
                console.log()
                ...
                await gemTokenChallengeFacet.connect(owner).setGameParameters(1118, "Red", 2, [50, 50])
                ...
                assert.strictEqual(winningValue, 1118)
                assert.strictEqual(primaryColor, "Red")
                assert.strictEqual(numberOfWinningSpot, 2)
                assert.strictEqual(isGameOpen, true)
                assert.deepStrictEqual(
                    gamePayoutSpots.map((n) => Number(n)),
                    [50, 50],
                ...
                expect(Number(await bankFacet.connect(signers[10]).getRewardsBalance())).to.equal(500 * 10 ** 6)
                expect(Number(await bankFacet.connect(signers[11]).getRewardsBalance())).to.equal(500 * 10 ** 6)
                expect(Number(await bankFacet.connect(signers[12]).getRewardsBalance())).to.equal(0)
                expect(Number(await bankFacet.connect(signers[13]).getRewardsBalance())).to.equal(0)
                expect(Number(await bankFacet.connect(signers[14]).getRewardsBalance())).to.equal(0)
                expect(Number(await bankFacet.connect(signers[15]).getRewardsBalance())).to.equal(0)
