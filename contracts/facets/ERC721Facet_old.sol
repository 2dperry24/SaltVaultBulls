...
//         });
//         s.erc721tokenIndividualApprovals[tokenId].push(record);
...
//         _erc721setApprovalForAll(auth, operator, approved);
//     }
...
//     //     _erc721beforeTokenTransfer(owner, address(0), tokenId, 1);

//     //     // Update ownership in case tokenId was transferred by `_erc721beforeTokenTransfer` hook
//     //     owner = s.erc721owners[msg.sender][tokenId];
...
//     //         s.erc721balances[msg.sender][owner] -= 1;
//     //     }
//     //     delete s.erc721owners[msg.sender][tokenId];

//     //     emit Transfer(owner, address(0), tokenId);

//     //     _erc721afterTokenTransfer(owner, address(0), tokenId, 1);
//     // }
...
//     function _erc721setApprovalForAll(address auth, address operator, bool approved) internal virtual {
//         require(auth != operator, "ERC721: approve to caller");

//         bool found = false;
//         for (uint i = 0; i < s.erc721ownerOperatorApprovals[auth].length; i++) {
//             if (s.erc721ownerOperatorApprovals[auth][i].operator == operator) {
//                 found = true;
//                 if (approved) {
//                     // Update the existing record with the new timestamp
//                     s.erc721ownerOperatorApprovals[auth][i].timestamp = block.timestamp;
//                     s.erc721ownerOperatorApprovals[auth][i].isApprovalActive = true;
//                 } else {
//                     // Remove the record by shifting the remaining elements left
//                     for (uint j = i; j < s.erc721ownerOperatorApprovals[auth].length - 1; j++) {
//                         s.erc721ownerOperatorApprovals[auth][j] = s.erc721ownerOperatorApprovals[auth][j + 1];
//                     }
//                     s.erc721ownerOperatorApprovals[auth].pop();
//                 }
//                 break;
//             }
//         }
...
//             });
//             s.erc721ownerOperatorApprovals[auth].push(record);
//         }

//         // Update the operator approval state
//         s.erc721operatorApprovals[msg.sender][auth][operator] = approved;
...
//         LibSharedStructs.IndividualApprovalRecord[] memory approvals = s.erc721tokenIndividualApprovals[tokenId];
...
//     operatorApprovals = s.erc721ownerOperatorApprovals[owner];
...
//         _erc721beforeTokenTransfer(address(0), _addressToMintTo, indexToMint, 1);
...
//     function erc721getAvailableFreeGemStoneMints(address _owner) public view returns (uint256) {
...
//             s.gemTokenChallangeBalance += _totalCost - splitCost;
